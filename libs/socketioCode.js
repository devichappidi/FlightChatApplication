...
//ioChat.to(userSocket[socket.username]).emit('onlineStack',userStack);
    ...
    socket.on('chat-msg', function(data){
      ...
      eventEmitter.emit('save-chat',{msgFrom:socket.username,msgTo:data.msgTo,msg:data.msg,room:socket.room});
      //emits event to send chat msg to all clients.
currentChats=function(result)
{
console.log("id"+result._id);
      ioChat.to(socket.room).emit('chat-msg',{_id:result._id,msgFrom:socket.username,msg:data.msg,date:today});
}
    });
             ...
             .exec(function(err,result){
               if(err){
                 ...
                 for(var i = 0; i < result.length; i++){
                   userStack[result[i].email] = "Offline";
                 }
